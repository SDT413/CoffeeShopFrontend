...
const ProductSliderSelectedItemHeader:FC<IProductSliderItemHeader> = ({product, isActive, onSelectItem}) => {
    const {nextItem, prevItem} = useActions();
    return (
            <div className={styles.itemHeader}>
                {isActive && (
                    <div className={styles.arrows}>
                    <button onClick={() => prevItem(products.length)} className={styles.arrow} aria-label={'Previous item'}>
                        <ChevronLeftIcon fontSize={'3rem'}/>
                    </button>
                    <button onClick={() => nextItem(products.length)} className={styles.arrow} aria-label={'Next item'}>
